...
cc.Class({
  ...
  deleteTile() {
    ...
    for (let i = 0; i < Config.DelRules.length; i++) {
      ...
      let intersectArr = HexUtil.ArrIntersect(fulledTilesIndex, delRule);
      if (intersectArr.length > 0) {
        // 判断两数组是否相同，相同则将方块添加到待消除数组里
        const isReadyDel = HexUtil.CheckArrIsEqual(delRule, intersectArr);
        if (isReadyDel) {
  ...
  setSpriteFrame(hexes) {
    for (let index = 0; index < hexes.length; index++) {
      ...
      label.string = "x:" + Math.floor( node.x)  + "\ny:" + Math.floor(node.y);
